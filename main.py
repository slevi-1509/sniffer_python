...
def handle_packet(packet, network, config, new_devices):
    if ipaddress.ip_address(packet.payload.src) in network: 
        try:
            ...
            if DNS in packet: #and pack_sum["dst_port"] == 53:
                try:
                    ...
                    pack_sum["dns_query"] = None
                    pack_sum["dns_answer"] = None
            if pack_sum["src_mac"] not in new_devices.keys() and packet.haslayer(TCP):
            ...
            try:
                with open(log_file_path, "a+") as log_file:
                    ...
                    for line in log_file:
                        ...
                        line_dict = ast.literal_eval(new_line)
...
def main(interface, interval, packets_count, no_of_sessions, ports_scan, os_detect):
    ...
    try:
        ...
        for i in count(0):
            new_devices = {}
            sniff(iface=interface, filter=filter, prn=lambda pkt: handle_packet(pkt, network, config, new_devices), 
                    count=packets_count, store=0)
            if new_devices:
                newDeviceExists(new_devices, ports_scan, os_detect)
...
def startSniffer():
    ...
    def on_close():
        ...
        main(interface, data['interval'], data['no_of_packets'], data['no_of_sessions'], data['ports_scan'], data['os_detect'])
